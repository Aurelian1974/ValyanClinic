-- =============================================
-- Script: Recreare tabelă Medicamente_Nomenclator cu coloane exacte din Excel
-- Data: 2025-12-27
-- Descriere: Coloanele respectă exact structura nomenclatorului ANM
-- =============================================
...
IF OBJECT_ID('dbo.Medicamente_Nomenclator_Backup', 'U') IS NOT NULL
    DROP TABLE [dbo].[Medicamente_Nomenclator_Backup];

IF OBJECT_ID('dbo.Medicamente_Nomenclator', 'U') IS NOT NULL
BEGIN
    SELECT * INTO [dbo].[Medicamente_Nomenclator_Backup] FROM [dbo].[Medicamente_Nomenclator];
    DROP TABLE [dbo].[Medicamente_Nomenclator];
    PRINT 'Backup creat și tabelă veche ștearsă.';
END
GO

-- Creare tabelă nouă cu coloane exacte din Excel ANM
...
CREATE TABLE [dbo].[Medicamente_Nomenclator] (
    ...
    [DenumireComercială] NVARCHAR(500) NOT NULL,
    
    -- Col 3: DCI (Denumire Comună Internațională)
    [DCI] NVARCHAR(500) NULL,
    
    -- Col 4: Forma farmaceutica
    [FormaFarmaceutică] NVARCHAR(200) NULL,
    
    -- Col 5: Concentratie
    [Concentrație] NVARCHAR(200) NULL,
    ...
    [ActiuneTerapeutică] NVARCHAR(500) NULL,
...
ON [dbo].[Medicamente_Nomenclator] ([DenumireComercială], [DCI], [CodATC])
WHERE [Activ] = 1;
GO

PRINT 'Tabelă Medicamente_Nomenclator recreată cu coloane exacte din Excel.';
GO

-- Actualizare stored procedures
CREATE OR ALTER PROCEDURE [dbo].[Medicamente_Upsert]
    @CodCIM NVARCHAR(20),
    @DenumireComercială NVARCHAR(500),
    @DCI NVARCHAR(500) = NULL,
    @FormaFarmaceutică NVARCHAR(200) = NULL,
    @Concentrație NVARCHAR(200) = NULL,
    @FirmaTaraProducatoareAPP NVARCHAR(500) = NULL,
...
BEGIN
    ...
    BEGIN
        ...
        SET [DenumireComercială] = @DenumireComercială,
            [DCI] = @DCI,
            [FormaFarmaceutică] = @FormaFarmaceutică,
            ...
            [CodATC] = @CodATC,
            [ActiuneTerapeutică] = @ActiuneTerapeutică,
    ...
    BEGIN
        INSERT INTO [dbo].[Medicamente_Nomenclator] (
            [CodCIM], [DenumireComercială], [DCI], [FormaFarmaceutică], [Concentrație],
            [FirmaTaraProducatoareAPP], [FirmaTaraDetinatoareAPP], [CodATC], [ActiuneTerapeutică],
...
BEGIN
    ...
    SELECT TOP (@MaxResults)
        [Id], [CodCIM], [DenumireComercială], [DCI], [FormaFarmaceutică], [Concentrație],
        [FirmaTaraProducatoareAPP], [FirmaTaraDetinatoareAPP], [CodATC], [ActiuneTerapeutică],
        [Prescriptie], [NrDataAmbalajAPP], [Ambalaj], [VolumAmbalaj], [ValabilitateAmbalaj],
        [Bulina], [Diez], [Stea], [Triunghi], [Dreptunghi], [DataActualizare],
    ...
    WHERE [Activ] = 1
      AND (
          [DenumireComercială] LIKE '%' + @SearchTerm + '%'
          OR [DCI] LIKE '%' + @SearchTerm + '%'
          OR [CodCIM] LIKE '%' + @SearchTerm + '%'
          OR [CodATC] LIKE '%' + @SearchTerm + '%'
      )
    ORDER BY 
        CASE WHEN [DenumireComercială] LIKE @SearchTerm + '%' THEN 0 ELSE 1 END,
        [DenumireComercială];
END
GO
...
-- Șterge backup dacă totul a mers bine
-- DROP TABLE [dbo].[Medicamente_Nomenclator_Backup];

  ...
  try {
    ...
    try {
      pool = await sql.connect(
        `Server=DESKTOP-3Q8HI82\\ERP;Database=ValyanMed;Trusted_Connection=Yes;TrustServerCertificate=Yes;`
      );
      return pool;
    } catch (err2) {
      throw new Error(
        `Nu s-a putut conecta la DESKTOP-3Q8HI82\\ERP / ValyanMed: ${err2}`
      );
    }
  }
}

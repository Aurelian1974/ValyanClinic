...
const tools: Tool[] = [
  // ── LIST / EXPLORE ──────────────────────────────────────────────────
  {
    name: 'list_objects',
    description:
      'Listează obiectele din baza de date ValyanMed. Tipuri: TABLE, VIEW, PROCEDURE, FUNCTION, TRIGGER, INDEX, SCHEMA, TYPE, SYNONYM. Poate filtra după schema și pattern de nume.',
    inputSchema: {
      ...
      properties: {
        ...
        schemaFilter: {
          type: 'string',
          description:
            'Filtrare opțională după schema (ex: dbo, med). Default: toate schemele.',
        },
        namePattern: {
          type: 'string',
          description:
            'Pattern opțional pentru filtrare după nume (LIKE pattern, ex: %Pacient%)',
        },
      },
      required: ['objectType'],
    },
  },
  {
    ...
    description:
      'Obține definiția/DDL-ul complet al unui obiect din baza de date (stored procedure, view, function, trigger). Pentru tabele, returnează structura completă cu coloane, tipuri, constrainte, indecși.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        objectName: {
          type: 'string',
          description:
            'Numele obiectului (cu sau fără schema, ex: dbo.Pacient sau Pacient)',
        },
        objectType: {
          type: 'string',
          enum: ['TABLE', 'VIEW', 'PROCEDURE', 'FUNCTION', 'TRIGGER', 'AUTO'],
          description:
            'Tipul obiectului. Folosește AUTO pentru detectare automată.',
          default: 'AUTO',
        },
      },
      required: ['objectName'],
  ...
  },
  {
    ...
    description:
      'Obține detalii complete despre un tabel: coloane, tipuri de date, constrainte (PK, FK, UNIQUE, CHECK, DEFAULT), indecși, și relații.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        tableName: {
  ...
  },
  {
    ...
    description:
      'Obține dependențele unui obiect - ce obiecte depind de el și de ce obiecte depinde.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        objectName: {
  ...
  // ── CREATE ──────────────────────────────────────────────────────────
  {
    name: 'create_object',
    description:
      'Creează un obiect nou în baza de date ValyanMed. Execută instrucțiuni CREATE (TABLE, VIEW, PROCEDURE, FUNCTION, TRIGGER, INDEX, TYPE, SCHEMA). Validează sintaxa înainte de execuție.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        sql: {
          type: 'string',
          description:
            'Instrucțiunea SQL CREATE completă. Exemplu: CREATE TABLE dbo.TestTable (Id INT PRIMARY KEY, Name NVARCHAR(100))',
        },
        description: {
          type: 'string',
  ...
  // ── MODIFY ──────────────────────────────────────────────────────────
  {
    name: 'modify_object',
    description:
      'Modifică un obiect existent în baza de date ValyanMed. Execută instrucțiuni ALTER (TABLE, VIEW, PROCEDURE, FUNCTION, TRIGGER). Pentru stored procedures și funcții, poate folosi CREATE OR ALTER.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        sql: {
          type: 'string',
          description:
            'Instrucțiunea SQL ALTER sau CREATE OR ALTER completă.',
        },
        description: {
          type: 'string',
  ...
  // ── OPTIMIZE ────────────────────────────────────────────────────────
  {
    ...
    description:
      'Analizează fragmentarea indecșilor pentru un tabel sau toată baza de date. Returnează procentul de fragmentare și recomandări (REORGANIZE < 30%, REBUILD > 30%).',
    inputSchema: {
      type: 'object' as const,
      properties: {
        tableName: {
          type: 'string',
          description:
            'Numele tabelului de analizat. Lasă gol pentru toată baza de date.',
        },
        minFragmentation: {
          type: 'number',
          description:
            'Fragmentare minimă % pentru filtrare (default: 5). Indecșii cu fragmentare sub acest prag nu sunt afișați.',
          default: 5,
        },
      },
    },
  },
  {
    name: 'optimize_index',
    description:
      'Optimizează un index specific prin REORGANIZE (fragmentare < 30%) sau REBUILD (fragmentare > 30%). Poate optimiza și toate indecșii unui tabel.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        tableName: {
        ...
        indexName: {
          type: 'string',
          description:
            'Numele indexului. Lasă gol pentru a optimiza toți indecșii tabelului.',
        },
        action: {
          type: 'string',
          enum: ['REORGANIZE', 'REBUILD', 'AUTO'],
          description:
            'Acțiunea de optimizare. AUTO alege automat pe baza fragmentării.',
          default: 'AUTO',
        },
      },
      required: ['tableName'],
  ...
  },
  {
    ...
    description:
      'Actualizează statisticile pentru un tabel sau toată baza de date. Statisticile actuale ajută query optimizer-ul să aleagă planuri de execuție optime.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        tableName: {
          type: 'string',
          description:
            'Numele tabelului. Lasă gol pentru a actualiza toate statisticile.',
        },
        fullScan: {
          type: 'boolean',
          description:
            'Dacă să folosească FULLSCAN (mai precis dar mai lent). Default: false.',
          default: false,
        },
      },
    },
  },
  {
    ...
    description:
      'Analizează indecșii lipsă sugerați de SQL Server pe baza query-urilor executate. Returnează recomandări cu impact estimat.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        tableName: {
          type: 'string',
          description:
            'Filtrare opțională pentru un tabel specific.',
        },
        minImpact: {
          type: 'number',
          description:
            'Impactul minim al indexului (default: 100). Valori mai mari = recomandări mai importante.',
          default: 100,
        },
      },
    },
  },
  {
    ...
    description:
      'Obține planul de execuție estimat (XML) pentru o interogare SQL, util pentru optimizare.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        sql: {
  ...
  // ── DELETE (cu confirmare) ──────────────────────────────────────────
  {
    name: 'prepare_delete',
    description:
      'PASUL 1 pentru ștergere: Analizează ce se va șterge, verifică dependențele și returnează un raport complet. NU șterge nimic - doar pregătește raportul. Utilizatorul trebuie să confirme apoi cu execute_delete.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        objectName: {
  ...
  },
  {
    name: 'execute_delete',
    description:
      'PASUL 2 pentru ștergere: Execută ștergerea obiectului DOAR după ce utilizatorul a confirmat. ATENȚIE: Această acțiune este IREVERSIBILĂ. Trebuie apelat prepare_delete mai întâi pentru a vedea dependențele.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        objectName: {
        ...
        parentTable: {
          type: 'string',
          description:
            'Pentru INDEX și TRIGGER - numele tabelului părinte',
        },
        confirmed: {
          type: 'boolean',
          description:
            'Trebuie să fie TRUE pentru a executa ștergerea. Setează pe TRUE doar dacă utilizatorul a confirmat explicit.',
        },
      },
  ...
  // ── UTILITY ─────────────────────────────────────────────────────────
  {
    name: 'run_sql',
    description:
      'Execută o interogare SQL arbitrară (SELECT, EXEC, etc.) pe baza de date ValyanMed. Pentru operații DDL (CREATE, ALTER, DROP), folosește tool-urile specifice.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        sql: {
  ...
  },
  {
    ...
    description:
      'Obține informații generale despre baza de date ValyanMed: dimensiune, număr obiecte, recovery model, compatibilitate, etc.',
    inputSchema: {
      type: 'object' as const,
      properties: {},
    },
...
async function handleGetObjectDefinition(args: {
  ...
  if (!objectType || objectType === 'AUTO') {
    ...
    if (typeDesc.includes('TABLE')) objectType = 'TABLE';
    else if (typeDesc.includes('VIEW')) objectType = 'VIEW';
    else if (typeDesc.includes('PROCEDURE')) objectType = 'PROCEDURE';
    else if (typeDesc.includes('FUNCTION')) objectType = 'FUNCTION';
    else if (typeDesc.includes('TRIGGER')) objectType = 'TRIGGER';
  ...
  try {
    ...
    return helpResult.recordset.map((r: any) => r.Text).join('');
  } catch {
    return `Nu s-a putut obține definiția pentru '${objectName}'.`;
  }
}

async function getTableDefinition(tableName: string): Promise<string> {
  const colQuery = `
...
async function handleCreateObject(args: {
  ...
  try {
    ...
    const objectMatch = sql.match(
      /CREATE\s+(?:OR\s+ALTER\s+)?(?:TABLE|VIEW|PROC(?:EDURE)?|FUNCTION|TRIGGER|INDEX|TYPE|SCHEMA|NONCLUSTERED\s+INDEX|CLUSTERED\s+INDEX|UNIQUE\s+INDEX)\s+(?:\[?(\w+)\]?\.)?(?:\[?(\w+)\]?)/i
    );
...
async function handleOptimizeIndex(args: {
  ...
  if (indexName) {
    ...
    try {
      await executeQuery(
        `ALTER INDEX [${indexName}] ON ${tableName} ${finalAction}`
      );
  ...
  try {
    ...
    await executeQuery(
      `ALTER INDEX ALL ON ${tableName} ${allAction}`
    );
...
async function handleAnalyzeQueryPlan(args: {
  ...
  try {
    ...
    try { await executeQuery('SET SHOWPLAN_XML OFF'); } catch {}
    return `❌ Eroare: ${err.message}`;
  }
}
...
async function handlePrepareDelete(args: {
  ...
  if (!objectType || objectType === 'AUTO') {
    ...
    const typeDesc: string = detected.recordset[0].type_desc;
    if (typeDesc.includes('TABLE')) objectType = 'TABLE';
    else if (typeDesc.includes('VIEW')) objectType = 'VIEW';
    else if (typeDesc.includes('PROCEDURE')) objectType = 'PROCEDURE';
    else if (typeDesc.includes('FUNCTION')) objectType = 'FUNCTION';
    else if (typeDesc.includes('TRIGGER')) objectType = 'TRIGGER';
    else objectType = typeDesc;
  }
...
async function handleRunSql(args: { sql: string }): Promise<string> {
  try {
    const result = await executeQuery(args.sql);
...
async function handleGetDatabaseInfo(): Promise<string> {
  const query = `
    SELECT 
      DB_NAME() AS [Database],
...
function formatResultAsTable(rows: any[]): string {
  if (!rows || rows.length === 0) return '(niciun rezultat)\n';
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      case 'list_objects':
        result = await handleListObjects(args as any);
        break;
      case 'get_object_definition':
        result = await handleGetObjectDefinition(args as any);
        break;
      case 'get_table_details':
        result = await handleGetTableDetails(args as any);
        break;
      ...
      case 'create_object':
        ...
        break;
      case 'modify_object':
        result = await handleModifyObject(args as any);
        break;
      case 'analyze_index_fragmentation':
        result = await handleAnalyzeIndexFragmentation(args as any);
        break;
      case 'optimize_index':
        result = await handleOptimizeIndex(args as any);
        break;
      case 'update_statistics':
        result = await handleUpdateStatistics(args as any);
        break;
      case 'analyze_missing_indexes':
        result = await handleAnalyzeMissingIndexes(args as any);
        break;
      case 'analyze_query_plan':
        result = await handleAnalyzeQueryPlan(args as any);
        break;
      case 'prepare_delete':
        result = await handlePrepareDelete(args as any);
        break;
      case 'execute_delete':
        result = await handleExecuteDelete(args as any);
        break;
      case 'run_sql':
        result = await handleRunSql(args as any);
        break;

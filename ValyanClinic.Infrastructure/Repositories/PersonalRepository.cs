...
public class PersonalRepository : IPersonalRepository
{
    ...
    public async Task<Personal> CreateAsync(Personal personal, string creatDe)
    {
        try
        {
            ...
            return MapDtoToPersonal(result);
        }
        catch (Exception ex)
        {
    ...
    private async Task EnsureConnectionOpenAsync()
    {
        try
        {
            if (_connection.State == ConnectionState.Closed)
            {
                ...
                _connection.Open(); // FIXED: Use synchronous Open() instead of OpenAsync()
            }
            ...
            else if (_connection.State == ConnectionState.Open)
            {
                ...
                try
                {
                    await _connection.QuerySingleAsync<int>("SELECT 1");
                }
                catch
                {
    ...
    private static DynamicParameters MapPersonalToParameters(Personal personal)
    {
        ...
        parameters.Add("@Nationalitate", personal.Nationalitate);
        parameters.Add("@Cetatenie", personal.Cetatenie);
